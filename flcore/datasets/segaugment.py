...
class ToTensor(object):
    ...
    def __call__(self, image, label):
        ...
        image = torch.from_numpy(image.transpose((2, 0, 1))[np.newaxis]).div(255) #LDP: ->[0,1] like F.to_tensor()
...
class Crop(object):
    ...
    def __init__(self, size, crop_type="center", ignore_index=255):
        ...
        if isinstance(ignore_index, int):
            ...
            raise (RuntimeError("ignore_index should be an integer number\n"))
    ...
    def __call__(self, image, label):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            label = F.pad(label, border, mode="constant", value=self.ignore_index)
        h, w = image.size()[-2:]
...
class RandRotate(object):
    ...
    def __init__(self, rotate, ignore_index=255):
    ...
    def __call__(self, image, label):
        ...
        label[label == 0.0] = self.ignore_index + 1
        label -= 1
        return image, label
...
def seg_augment(name, split):
    img_size = INFO[name]['shape'][1:]
    configs = {
        'train':{
            'flip': True,
            'GaussianBlur': False,
    ...
    cfg = configs[split]
    transform = []
    moments = INFO[name]['moments']
    mean, std = moments[0], moments[1]
    ignore_index = INFO[name]["ignore_index"]
    transform.append(ToTensor())
    transform.append(Normalize(mean=mean, std=std))
    if cfg.get("resize", False):
        transform.append(Resize(cfg["resize"]))
    if cfg.get("rand_resize", False):
        transform.append(RandResize(cfg["rand_resize"]))
    if cfg.get("rand_rotation", False):
        rand_rotation = cfg["rand_rotation"]
        transform.append(
            RandRotate(rand_rotation, ignore_index=ignore_index)
        )
    if cfg.get("GaussianBlur", False) and cfg["GaussianBlur"]:
        transform.append(RandomGaussianBlur())
    ...
    if cfg.get("crop", False):
        ...
        transform.append(
            Crop(crop_size, crop_type=crop_type, ignore_index=ignore_index)
        )
