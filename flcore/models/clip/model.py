...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None,
        ...
        self.width = width
        self.layers = layers
        if design_details is not None and design_details['trainer'] == 'MaPLe':
    ...
    def forward_tpg(self, x, prompts=[], text_prompt=True):
        # return self.resblocks(x)
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts):
        ...
        if self.VPT_shallow:
            visual_ctx = shared_ctx.expand(x.shape[0], -1, -1) # fix shape mismatch bug
            # visual_ctx = shared_ctx.repeat(x.shape[0]//shared_ctx.shape[0], 1, 1)
